...
- Realtime ranking improved using tick-based micro-momentum (5-minute only).
- "Momentum" metric includes RFactor contribution.
- Ranking is done by `momentum` (internal `_rank` key), not the old `score`.
"""
...
def _session_trend_quality(
    ...
    if today.empty:
        return 1.0
    values = [_as_float(today.iloc[0]["open"])] + [_as_float(value) for value in today["close"].tolist()]
...
def _volume_confirmation(
    ...
    if live_candles and any(candle["volume"] > 0 for candle in live_candles):
        ...
        directional_rate = sum(
            move >= 0.02 and volume >= baseline * 0.80 for move, volume in zip(moves, volumes)
        ) / len(recent_candles)
...
# -------- NEW: tick-based micro-momentum helpers (5-min only used) --------

def _micro_change(token: int, lookback_sec: int, now_ts: Optional[float] = None) -> Optional[float]:
    """% change between latest tick and the last tick at/older than now-lookback_sec."""
...
def _build_row(symbol: str, sector: str, timeframe: str) -> Optional[dict]:
    ...
    # Base scoring (kept internal; ranking uses `momentum`)
    stale_trend = (
        abs(change) * 0.40
        + effective_ratio * 1.6
    ...
    # --- tick-based micro-momentum (5 minutes only, intraday) ---
    micro_factor = 1.0
    micro_5m = None
    if timeframe == "intraday":
        now_ts = time.time()
        micro_5m = _micro_change(token, 300, now_ts)  # 5 minutes
        if micro_5m is not None:
            aligned_micro = expected_direction * micro_5m
            # map roughly +/-1% aligned move into ~[0.7..1.3]
            micro_factor = max(0.70, min(1.30, 1.0 + aligned_micro / 2.0))

    # --- momentum includes rfactor + micro-momentum ---
    rfactor_mult = 1.0 + max(0.0, float(rfactor)) / 8.0
    momentum = score * rfactor_mult * micro_factor
    ...
    return {
        ...
        "isIndex": False,
        "_rank": float(momentum),  # internal sort key
    }
...
def _aggregate_index(name: str, rows: List[dict]) -> Optional[dict]:
    ...
    strongest = max(rows, key=_rank_value)
...
def scan():
    ...
    if sector_upper == "ALL":
        ...
        sector_key = sector_upper.lower()
    else:
        sector_key = "all"

    rows = build_rows(timeframe, universe, sector_key)
    status, live = _feed_status()
    return jsonify(
        {
            "live": live,
